...
![](/images/audit/f069e0b48b1c418c98a2fa06f340f7e1.png)
...
![](/images/audit/9360962f02bb46cd9ba3bba00d044c67.png)

看一下我 D 盘下的文件，以这个 `123.txt`为例
![](/images/audit/f258554aa3f0497793ec5f7ff8dc188c.png)

### 手动触发定时任务
![](/images/audit/0327170c54ad49148a841406d48699b5.png)
...
![](/images/audit/2110056165da498d9e501d05a7eef5c8.png)
...
3. 尝试点击【执行一次】，发现控制台中成功输出内容，说明方法被正确调用
	![](/images/audit/861ce4c114014cd8a083a034344107d1.png)
...
6. 创建一个测试类，用 `@Controller` 注解标记尝试一下（修改完代码要重启服务）
	...
	```
7. 添加定时任意，尝试调用测试类
	![](/images/audit/112f53de1c7b462ab26d05b17921ee79.png)
	![](/images/audit/23a2342cfcf542b6ab193bdd2d5eaec3.png)
	进入了断点，并成功输出了 `helloruoyi`，由此验证，上面的猜想是正确的
	![](/images/audit/d5f789b5856040b59c632fffa06c8d93.png)
...
9. `ruoYiConfig.setProfile` 的作用
	...
	![](/images/audit/6d6766f5913b4ef8b59b9fea4586007f.png)
	![](/images/audit/a387b929de844230afb621bcf06d7c39.png)
10. 文件保存的默认目录可以被修改，那，被修改之后呢？还需要找一个访问这个目录下内容的方法。

	> 走读代码，在 `CommonController.java` 中，找到一个 `resourceDownload` 的方法
	
	![](/images/audit/fbc3ec13c651465cb9ba715744f03b47.png)

	> 该方法内，先是进行了非法路径的检查

	![](/images/audit/e55c7da7f9434e49b1592c5de10d38bb.png)
	![](/images/audit/cff86d9c7a4242e4888ed5d711c6d593.png)
	同时里面还有一个白名单（`MimeTypeUtils.DEFAULT_ALLOWED_EXTENSION`）校验
	![](/images/audit/42f342d67bf142efa7b35ee233d38b19.png)
...
- 按照前 9 步的分析，我目前测试机是 `Windows` 机器，所以编写 `payload` 为 `ruoYiConfig.setProfile("D:\\")`，尝试将文件默认路径设置为 `D` 盘根目录
	![](/images/audit/015be50991394b4c8078dd2f1c4546a1.png)
- 点击执行，手动触发定时任务
	![](/images/audit/f5697e2d1b454020b38b74df44c7b7b3.png)
- 构造下载链接，访问 D 盘中的资源
...
- 下载成功
	![](/images/audit/fc6e16e2e30640c0938fc15ab499be73.png)
	
## 漏洞修复
> 截止发文日（2023.05.25），Ruoyi 官方已经于 2023.4.14 日针对 v4.7.6 版本的任意文件下载漏洞进行了修复。

![](/images/audit/cd4818ccad4c4dca95392bbf7d2933c7.png)
通过Compare v4.7.7 和 v4.7.6 的代码变动情况，可以看出，官方的修复方案如下：
![](/images/audit/877ee5e20d094a3db25c8e6699317111.png)
尝试在 v4.7.7 版本中复现，结果被成功拦截

![](/images/audit/49208e1b0a994b0594f31c2efc603b84.png)
